...
class AGMBlock(nn.Module):
    ...
    def __init__(self, c1: int, c2: int | None = None, num_heads: int = 4, reduction: int = 16):
        """Initialize AGM block for single-point neck enhancement with shape-preserving ops.

        Notes:
            parse_model() treats AGMBlock as a base module and passes (c1, c2, ...).
            AGM is shape-preserving, so c2 must equal c1 when provided.
        """
        super().__init__()
        c2 = c1 if c2 is None else c2
        if c2 != c1:
            raise ValueError(f"AGMBlock requires c1 == c2 for shape-preserving insertion, got c1={c1}, c2={c2}")
